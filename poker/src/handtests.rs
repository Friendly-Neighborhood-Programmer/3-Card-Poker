...
impl HandTests {
  pub fn test_for_straight(deck: &Deck) -> bool {
    for i in 0..2 {
      let currCard: &Card = deck.get_cards().get(i).unwrap();
      let nextCard:&Card = deck.get_cards().get(i + 1).unwrap();
      if(nextCard.get_value() - currCard.get_value() != 1) {
        return false;
      }
    }
    true  
  ...
  pub fn test_for_flush(deck: &Deck) -> bool {
      for i in 0..2 {
        let currCard: &Card = deck.get_cards().get(i).unwrap();
        let nextCard:&Card = deck.get_cards().get(i + 1).unwrap();
        if nextCard.get_suit() != currCard.get_suit() {
          return false;
        }
      }
      return true;
